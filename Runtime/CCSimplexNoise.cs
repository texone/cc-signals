...
namespace cc.creativecomputing.math.signal
{
	...
	public class CCSimplexNoise : CCSignal
	{
		...
		public CCSimplexNoise() : base()
		{
		}

		public CCSimplexNoise(CCSignalSettings theSettings) : base(theSettings)
		{
		}
		...
		public override float[] noiseImpl(float x)
		{
			...
			return new float[] {_mySettings.normed ? (myNoise + 1) / 2 : myNoise, _mySettings.normed ? (dx + 1) / 2 : dx};
		}
		...
		/// @return </param>
		{
			...
			return new float[] {(Mathf.Clamp(Noise * 70.1605f - 0.000142584f, -1.0f, 1.0f) + 1) / 2, (dx + 1) / 2, (dy + 1) / 2};
		}
		...
		/// @return </param>
		{
			...
			return new float[] {(Mathf.Clamp(Noise * 32.741f + 0.00104006f, -1.0f, 1.0f) + 1) / 2, (dx + 1) / 2, (dy + 1) / 2, (dz + 1) / 2};
		}
		...
		public virtual float[] noiseImpl(float x, float y, float z, float w)
		{
			...
			return new float[] {_mySettings.normed ? (myNoise + 1) / 2 : myNoise, _mySettings.normed ? (dx + 1) / 2 : dx, _mySettings.normed ? (dy + 1) / 2 : dy, _mySettings.normed ? (dz + 1) / 2 : dz, _mySettings.normed ? (dw + 1) / 2 : dw};
		}
	}
